-- ============================================
-- INSTRUCCIONES:
-- 1. Primero crear la base de datos manualmente:
--    psql -U postgres
--    CREATE DATABASE ventas;
--    \q
-- 
-- 2. Luego ejecutar este script:
--    psql -U postgres -d ventas -f crear_tablas.sql
-- ============================================
...
DROP TRIGGER IF EXISTS trigger_actualizar_stock ON detalle_ventas;
...
DROP TRIGGER IF EXISTS trigger_calcular_subtotal ON detalle_ventas;
...
SELECT 'Administrador', 'admin@ventas.com', '$2a$10$XqY9z4jK9vZ5gN2mP7rO6eJkL8wH3nF4vB6tC2dE1fG5hI7jK8lM9', 'admin'
WHERE NOT EXISTS (SELECT 1 FROM usuarios WHERE email = 'admin@ventas.com');

INSERT INTO usuarios (nombre, email, password, rol) 
SELECT 'Juan Pérez', 'juan@ventas.com', '$2a$10$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ012', 'usuario'
WHERE NOT EXISTS (SELECT 1 FROM usuarios WHERE email = 'juan@ventas.com');

-- Insertar productos (solo si no existen)
INSERT INTO productos (nombre, descripcion, precio, stock)
SELECT 'Laptop HP', 'Laptop HP 15.6" Intel Core i5 8GB RAM', 599.99, 15
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE nombre = 'Laptop HP');

INSERT INTO productos (nombre, descripcion, precio, stock)
SELECT 'Mouse Logitech', 'Mouse inalámbrico Logitech M185', 15.99, 50
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE nombre = 'Mouse Logitech');

INSERT INTO productos (nombre, descripcion, precio, stock)
SELECT 'Teclado Mecánico', 'Teclado mecánico RGB retroiluminado', 79.99, 25
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE nombre = 'Teclado Mecánico');

INSERT INTO productos (nombre, descripcion, precio, stock)
SELECT 'Monitor Samsung', 'Monitor Samsung 24" Full HD', 189.99, 10
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE nombre = 'Monitor Samsung');

INSERT INTO productos (nombre, descripcion, precio, stock)
SELECT 'Auriculares Sony', 'Auriculares inalámbricos Sony WH-1000XM4', 299.99, 8
...
-- Usuario: juan@ventas.com / usuario123
